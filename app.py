...
def tag_mp3_file(filepath, metadata, image_data=None):
    try:
        ...
        if image_data:
            mime_type = imghdr.what(None, h=image_data)
            if mime_type:
                audio.tags.add(
                    APIC(
                        encoding=3,
                        mime=f'image/{mime_type}',
                        type=3,
                        desc='Cover',
                        data=image_data
                    )
...
def extract_text_and_metadata(filepath):
    ...
    try:
        if extension == '.pdf':
            ...
            first_image = next(images, None)
            if first_image:
                image_data = first_image.get_content()
        elif extension == '.txt':
...
def convert_to_speech_task(self, input_filepath, original_filename, voice_name=None):
    try:
        self.update_state(state='PROGRESS', meta={'current': 1, 'total': 4, 'status': 'Extracting text and cover art...'})
        text_content, metadata, image_data = extract_text_and_metadata(input_filepath)
        ...
        tag_mp3_file(output_filepath, metadata, image_data)
