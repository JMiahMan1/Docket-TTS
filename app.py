...
def tag_mp3_file(filepath, metadata, image_data=None):
    """
    Adds ID3 tags to an MP3 file. Handles empty files for testing and
    embeds cover art if provided.
    """
    try:
        # Try to load existing tags, or create a new ID3 object if the file is empty/invalid
        try:
            tags = ID3(filepath)
        except Exception:
            tags = ID3()
        ...
        # Add standard text tags
        tags.add(TIT2(encoding=3, text=title))
        tags.add(TPE1(encoding=3, text=author))
        tags.add(TALB(encoding=3, text=title)) # Album tag
        tags.add(COMM(encoding=3, lang='eng', desc='Comment', text='Generated by Docket TTS'))
...
def convert_to_speech_task(self, input_filepath, original_filename, voice_name=None):
    try:
        ...
        tts.synthesize(text_content, output_filepath)
        ...
        # In a real app, you would get the normalized text from the TTSService
        # For now, we write the original extracted text.
        Path(text_filepath).write_text(text_content, encoding="utf-8")
...
def _create_audiobook_logic(file_list, audiobook_title, audiobook_author, cover_url, build_dir, task_self=None):
    ...
    for i, path in enumerate(safe_mp3_paths):
        try:
            ...
            duration_s = 0 # Fallback for files that can't be read
        duration_ms = int(duration_s * 1000)
        concat_list_content += f"file '{path.resolve()}'\n"
        chapter_metadata_list.append({
            "title": f"Chapter {i + 1}",
            "start_time": current_duration_ms,
            "end_time": current_duration_ms + duration_ms
        })
